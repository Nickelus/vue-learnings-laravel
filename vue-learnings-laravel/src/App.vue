<template>
  <div>
    <div
      @click="editedPost = {}"
      v-if="Object.keys(editedPost).length > 0"
      class="modal-shadow"
    >
      <div @click.stop class="modal">
        <div class="modal-header mb-4">
          <div>
            <h3 class="text-lg">Edit post with title:</h3>
          </div>
        </div>
    ...
    <div class="w-1/2 mx-auto p-4">
      ...
      <div>
        <div
          ...
          class="p-4 mb-4 bg-white border border-gray-200 flex justify-between items-center"
        >
          <div>
            <h3 class="text-lg text-gray-800 mb-2 border-b border-gray-200">
              {{ postItem.title }}
            </h3>
          ...
          <div class="flex items-center">
            <svg
              @click="editPost(postItem)"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="size-6 mr-2 cursor-pointer text-emerald-600"
            >
              <path
